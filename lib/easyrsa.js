...
async function easyrsa(environment, ...args) {
    ...
    try {
        const { stdout, stderr } = await exec(commandWithArgs, { cwd: easyrsa3Path, env: commandEnvironment });
        debug(`stdout: ${stdout}`);
        debug(`stderr: ${stderr}`);
        return;
    }
...
module.exports.checkPKI = async function checkPKI() {
    ...
    try {
        await fs.access(pkiPath, fs.constants.F_OK);
        return true;
    }
    catch (e) {
        debug('PKI missing');
...
module.exports.buildCA = async function buildCA() {
    ...
    if (pkiExists) {
        throw new Error('You cannot initialize the CA since a pki directory already exists in the project.  If you really want to do this you will need to remove the pki directory first.');
    }

    debug('Checking CA private key existence...');
    const caPrivateKeyExists = await secrets.doesCAPrivateKeyExist();
    if (caPrivateKeyExists) {
        throw new Error('You cannot initialize the CA since a private key already exists in Secrets Manager.  If you really want to do this you will need to eliminate the private key from secrets manager first.');
    }

    debug('Initializing pki');
    await easyrsa(null, 'init-pki');
    debug('Build out the CA and create a new private key')
    await easyrsa({ EASYRSA_REQ_CN: 'gitcaws' }, 'build-ca', 'nopass',);
    debug('reading ca cert');
    const caCert = await fs.readFile(path.join(pkiPath, `ca.crt`), "utf8");
    debug('reading ca private key');
    const caPrivateKey = await fs.readFile(path.join(pkiPath, `private/ca.key`), "utf8");
    debug('Uploading certs to AWS');
    await certManager.importCertificate({
        Certificate: caCert,
        PrivateKey: caPrivateKey,
        CertificateChain: caCert
    }).promise();
    debug('Saving off private key');
    await secrets.saveAndDeleteCAPrivateKeyFromFileToSecretsManager();
};

module.exports.createServer = async function createServer() {
    const serverCN = 'gitcaws-server';
    try {
        ...
        if (serverPrivateKeyExists) {
            throw new Error('You cannot create a server certificate because one already exists in Secrets Manager.  If you really want to do this you will need to eliminate the private key from secrets manager first.');
        }
        ...
        debug('Saving off private key');
        await secrets.saveServerPrivateKeyFromFileToSecretsManager(serverCN);
        ...
        const cert = await fs.readFile(path.join(pkiPath, `issued/${serverCN}.crt`), "utf8");
        debug('reading ca cert');
        const caCert = await fs.readFile(path.join(pkiPath, `ca.crt`), "utf8");
        debug('Uploading certs to AWS');
        await certManager.importCertificate({
...
module.exports.revokeClient = async function revokeClient(commonName) {
    try {
        ...
        await ec2.importClientVpnClientCertificateRevocationList({
...
dev tun
proto tcp
remote ${commonName}.${vpnClientEndpointId}.prod.clientvpn.${process.env.AWS_REGION}.amazonaws.com 443
remote-random-hostname
resolv-retry infinite
nobind
persist-key
persist-tun
remote-cert-tls gitcaws-server
cipher AES-256-GCM
verb 3
<ca>
${caCert}
